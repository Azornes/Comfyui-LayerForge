...
if (!log) {
    log = {
        debug: (...args: any[]) => console.debug(`[${CONFIG.LOGGER_NAME}]`, ...args),
        info: (...args: any[]) => console.info(`[${CONFIG.LOGGER_NAME}]`, ...args),
        warn: (...args: any[]) => console.warn(`[${CONFIG.LOGGER_NAME}]`, ...args),
        error: (...args: any[]) => console.error(`[${CONFIG.LOGGER_NAME}]`, ...args)
    };
}
...
export function showNotification(
    message: string,
    typeOrBgColor: string | object | undefined = "info",
    durationOrOptions: number | object | undefined = 3000,
    typeArg?: "success" | "error" | "info" | "warning" | "alert",
    ...
    let type: "success" | "error" | "info" | "warning" | "alert" = "info";
    let duration: number = 3000;
    let deduplicate: boolean = false;
    let options: any = {};
    ...
    if (typeof typeOrBgColor === "object" && typeOrBgColor !== null) {
        ...
        if (typeof typeOrBgColor === "string") {
            if (typeOrBgColor.startsWith("#")) {
                ...
                type = (typeArg || "info") as any;
            } else {
                type = typeOrBgColor as any;
            }
        }
    ...
    if (deduplicate) {
        ...
        if (existingNotification) {
            ...
            const progressBar = existingNotification.element.querySelector('div[style*="scaleX"]') as HTMLDivElement;
            if (progressBar) {
    ...
    let container = document.getElementById(CONFIG.SHARED_CONTAINER_ID) as HTMLDivElement;
    if (!container) {
    ...
    const styleSheet = document.getElementById(`${CONFIG.CSS_PREFIX}notification-styles`) as HTMLStyleElement;
    if (!styleSheet) {
