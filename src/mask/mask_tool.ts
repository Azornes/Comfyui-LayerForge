...
import { createCanvas, createCanvasWithContext } from "../utils/common_utils.js";
import {
    applyLuminanceAsAlpha,
    calculateDistanceTransform,
    imageDataToBinaryMask,
    rasterizeDistanceFieldMask
} from "./mask_pixel_utils.js";
import type { Canvas } from '../canvas/canvas';
import type { Point, CanvasState, Layer } from '../shared/types';
...
export class MaskTool {
     ...
     */
    private forEachChunkInArea(area: MaskArea, operation: (chunk: MaskChunk) => void): void {
    ...
    private createFeatheredMaskFromBinaryData(binaryData: Uint8Array, featherRadius: number, width: number, height: number): HTMLCanvasElement {
        ...
        const distanceMap = calculateDistanceTransform(binaryData, width, height);
    ...
    handleMouseDown(worldCoords: Point, _viewCoords: Point): void {
     ...
     */
    private updateActiveCanvasIfNeeded(_startWorld: Point, _endWorld: Point): void {
    ...
    drawBrushPreview(_viewCoords: Point): void {
     ...
     */
    setMaskForLayer(image: HTMLImageElement, layer: Layer): void {
        ...
        log.info(
            `MaskTool set layer-aligned mask at (${maskLeft.toFixed(1)}, ${maskTop.toFixed(1)}) ` +
    ...
    addMask(image: HTMLImageElement): void {
        ...
        const activatedChunks = this.activateChunksInArea(
            maskArea.left,
            maskArea.top,
            maskArea.right,
            maskArea.bottom
