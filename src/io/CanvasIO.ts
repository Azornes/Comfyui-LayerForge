...
import { scaleImageToFit, loadImage, blobToDataUrl, tensorToImageData, createImageFromImageData } from "../media/ImageUtils.js";
import { postImageBlob } from "../media/ImageUploadUtils.js";
...
import type { Layer, Shape, AddMode } from '../shared/types';
...
export interface ConnectedInputImage {
...
function getImageSourceIdentity(source?: HTMLImageElement | string): string {
...
export class CanvasIO {
    ...
    private _inputDataCheckPromise: Promise<void> | null;
    ...
    getConnectedInputImages(): ConnectedInputImage[] {
        ...
        return this.getConnectedImageSources().flatMap(({ sourceNode, sourceId, sourceSlot, connectionType }) => {
            ...
            return (sourceNode.imgs as HTMLImageElement[]).map((image, imageIndex) => ({
                image,
                sourceId,
                sourceSlot,
                imageIndex,
    ...
    unlinkConnectedInputImage(
        reference: Pick<ConnectedInputImage, 'sourceId' | 'sourceSlot' | 'connectionType'>,
    ): boolean {
        const node = this.canvas.node as any;
        const sourceId = Number(reference.sourceId);
        const sourceSlot = Number(reference.sourceSlot);
        if (!Number.isFinite(sourceId) || !Number.isFinite(sourceSlot)) return false;

        if (reference.connectionType === 'virtual') {
            const links = getLayerForgeImageInputLinks(node);
            const linkIndex = links.findIndex(link => (
                link.source_id === sourceId && link.source_slot === sourceSlot
            ));
            if (linkIndex < 0 || !removeLayerForgeImageInputLink(node, linkIndex)) return false;
        } else {
            const input = this.getImageInputSlot();
            const linkId = input?.link;
            const nativeLink = this.getGraphLink(linkId);
            if (!input || linkId == null || !nativeLink) return false;
            ...
            if (nativeSourceId !== sourceId || nativeSourceSlot !== sourceSlot) return false;
        ...
        node.graph?.change?.();
        (globalThis as any).app?.graph?.change?.();
        this.canvas.render();
    ...
    private async addBatchImages(
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            const layerProps: Partial<Layer> = {
    ...
    private getCanvasImageIdentities(): Set<string> {
    ...
    async _performSave(fileName: string, outputMode: string): Promise<any> {
        ...
        return new Promise((resolve) => {
            ...
            const { canvas: tempCanvas } = this.canvas.canvasLayers.renderLayersToCanvas(
                renderBounds,
                this.canvas.layers,
                {}
            );
            const { canvas: maskCanvas, ctx: maskCtx } = this.canvas.canvasLayers.renderLayerVisibilityMask(
                renderBounds,
                this.canvas.layers,
                {
                    maskContextOptions: {},
    ...
    private async checkForInputDataInternal(options?: { allowImage?: boolean; allowMask?: boolean; reason?: string }): Promise<void> {
        ...
        try {
            ...
            if (allowImage && connectedImageSources.length > 0) {
                ...
                const currentSourceIdentities = connectedImageSources.map(({ sourceNode }) => imageBatchIdentity(
                    sourceNode.imgs.map((img: HTMLImageElement) => getImageSourceIdentity(img))
                ));
                ...
                if (!imageLoaded || imageChanged) {
                    ...
                    if (sourceImages.length > 0) {
                        ...
                        const sourceIdentities = connectedImageSources.map(({ sourceNode }) => imageBatchIdentity(
                            sourceNode.imgs.map((img: HTMLImageElement) => getImageSourceIdentity(img))
                        ));
                        ...
                        const addedCount = await this.addBatchImages(
                            sourceImages,
                            addMode,
    ...
    async addSelectedInputImage(image: HTMLImageElement): Promise<boolean> {
        try {
