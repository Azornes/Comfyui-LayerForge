import {saveImage, removeImage} from "../persistence/db.js";
...
import {showErrorNotification, showSuccessNotification} from "../utils/notification_utils.js";
...
import type { Layer, Point, AddMode, ClipboardPreference } from '../shared/types';
...
interface ProcessedImageFallback {
    cacheKey: string;
...
interface DragSceneStaticSegment {
    layers: Layer[];
...
interface DragScenePart {
    staticSegments: DragSceneStaticSegment[];
    dynamicLayers: Layer[];
}

interface DragSceneCache {
    key: string;
    staticLayers: Layer[];
...
export class CanvasLayers {
    ...
    private processedImageMetadata: Map<string, ProcessedImageMetadata> = new Map();
    private lastProcessedImageFallbacks: Map<string, ProcessedImageFallback> = new Map();
    ...
    private latestProcessedImageKeys: Map<string, string> = new Map();
     ...
     */
    private getProcessedImageGeometryKey(layer: Layer): string {
    ...
    public prepareDragLayerPreviews(layers: Layer[]): void {
        const previewOptions: DragPreviewOptions = {
    ...
    private isLatestProcessedImageKey(layerId: string, cacheKey: string): boolean {
        return this.latestProcessedImageKeys.get(layerId) === cacheKey;
    }

    private isProcessedImageReferenced(image: CanvasImageSource): boolean {
        for (const cachedImage of this.processedImageCache.values()) {
            if (cachedImage === image) return true;
        }
        for (const fallback of this.lastProcessedImageFallbacks.values()) {
            if (fallback.image === image) return true;
        }
        return false;
    }

    private releaseProcessedImageIfUnreferenced(image: CanvasImageSource | undefined): void {
        if (!image || this.isProcessedImageReferenced(image)) return;
        this.releaseProcessedImage(image);
    }

    private rememberProcessedImageFallback(layerId: string, cacheKey: string, image: CanvasImageSource): void {
    ...
    private setLatestProcessedImageKey(layer: Layer, cacheKey: string): void {
    ...
    private getProcessedImage(
        ...
        options: {
            cacheOnly?: boolean;
            allowCacheWhileAdjusting?: boolean;
            allowStaleCacheWhileDragging?: boolean;
        } = {}
    ): CanvasImageSource | null {
        const {
            cacheOnly = false,
            allowCacheWhileAdjusting = false,
            allowStaleCacheWhileDragging = false
        } = options;
        const blendArea = layer.blendArea ?? 0;
        const needsBlendAreaEffect = blendArea > 0;
     ...
     */
    public handleLayerDragEnd(layers: Layer[]): void {
        for (const layer of layers) {
            if (!layer.image) continue;
            ...
            if (!hasBlendArea && !hasCropEffect) continue;
    ...
    private setProcessedImageCache(layer: Layer, key: string, image: CanvasImageSource): void {
    ...
    private createDragStaticSegments(layers: Layer[]): DragSceneStaticSegment[] {
        const segments: DragSceneStaticSegment[] = [];
        let sourceOverLayers: Layer[] = [];

        const flushSourceOverLayers = () => {
            if (sourceOverLayers.length === 0) return;
    ...
    private buildDragSceneCache(
        ...
        const parts: DragScenePart[] = [];
        let staticLayers: Layer[] = [];
        let currentPart: DragScenePart = {
            staticSegments: [],
            dynamicLayers: []
        };

        const flushStaticLayers = () => {
            if (staticLayers.length === 0) return;
    ...
    private drawDragStaticSegment(ctx: CanvasRenderingContext2D, segment: DragSceneStaticSegment): void {
        if (!segment.canvas) return;

        ctx.save();
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.globalCompositeOperation = segment.blendMode as GlobalCompositeOperation;
        ctx.globalAlpha = segment.opacity;
        ctx.drawImage(segment.canvas, 0, 0);
        ctx.restore();
    }

    public clearDragSceneCache(): void {
        if (!this.dragSceneCache) return;
    ...
    public drawLayersDuringDrag(
        ...
        if (!this.dragSceneCache || this.dragSceneCache.key !== key || selectedReferencesChanged || staticReferencesChanged) {
            ...
            this.dragSceneCache = this.buildDragSceneCache(
                sortedLayers,
                selectedIds,
                selectedLayers,
                key,
    ...
    public renderLayersToCanvas(
        bounds: { x: number, y: number, width: number, height: number },
        layers: Layer[] = this.canvas.layers,
    ...
    public renderLayerVisibilityMask(
        bounds: { x: number, y: number, width: number, height: number },
        layers: Layer[] = this.canvas.layers,
        options: {
            maskContextOptions?: CanvasRenderingContext2DSettings;
            visibilityContextOptions?: CanvasRenderingContext2DSettings;
        } = {}
    ): { canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D } {
        const {
        ...
        const { ctx: visibilityCtx } = this.renderLayersToCanvas(
            bounds,
            layers,
            visibilityContextOptions
        );
    ...
    showBlendModeMenu(worldX: number, worldY: number): void {
        ...
        this.blendModes.forEach((mode: BlendMode) => {
            ...
            option.onclick = () => {
                ...
                content.querySelectorAll<HTMLDivElement>('.lf-blend-mode-container').forEach(c => {
                    c.classList.remove('lf-active');
                    const optionDiv = c.querySelector<HTMLDivElement>('.lf-blend-mode-option');
                    if (optionDiv) {
    ...
    private async _generateCanvasBlob(options: {
        ...
        return new Promise((resolve) => {
            ...
            const { canvas: tempCanvas, ctx: tempCtx } = this.renderLayersToCanvas(
                bounds,
                layers,
    ...
    async getFlattenedMaskAsBlob(): Promise<Blob | null> {
        return new Promise((resolve) => {
            ...
            const { canvas: maskCanvas, ctx: maskCtx } = this.renderLayerVisibilityMask(
                bounds,
                this.canvas.layers,
                {
                    maskContextOptions: { willReadFrequently: true },
